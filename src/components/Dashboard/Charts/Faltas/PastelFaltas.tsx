...
export const PastelFaltas = () => {
  const { filteredData } = useDashboard();

  const datosPieFaltasPorArea = useMemo(() => {
    if (filteredData.length === 0) return [];

    const faltasPorArea: Record<string, number> = {};

    // Sumar faltas por área
    filteredData.forEach((r) => {
      faltasPorArea[r.area] = (faltasPorArea[r.area] || 0) + r.faltas;
    });

    const total = Object.values(faltasPorArea).reduce((s, v) => s + v, 0);
    ...
    return Object.entries(faltasPorArea)
      .map(([area, value], i) => ({
  ...
  const totalFaltas = datosPieFaltasPorArea.reduce(
    (s, i) => s + i.value, 0
  );
  ...
  return (
    <div className="bg-white p-6 rounded-xl shadow-md">
      ...
      {datosPieFaltasPorArea.length === 0 ? (
        ...
        <div className="h-[400px] w-full flex flex-col lg:flex-row items-center lg:items-start gap-6">
          ...
          <div className="w-full lg:w-2/3 h-[300px] lg:h-[400px]">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                ...
                >
                  {datosPieFaltasPorArea.map((entry, index) => (
                    <Cell
                      key={`cell-${index}`}
                      fill={entry.color}
                      stroke="#fff"
                ...
                <PieTooltip
                  formatter={(value) => {
                    ...
                    const perc =
                      totalFaltas > 0
                        ? ((v / totalFaltas) * 100).toFixed(1)
                        : "0.0";
                    return `${v} faltas (${perc}%)`;
                  }}
                  contentStyle={{
          ...
          <div className="w-full lg:w-1/3 h-auto lg:h-[400px] overflow-y-auto pr-1">
            <div className="space-y-2">
              {datosPieFaltasPorArea.map((item, i) => (
                <div
                  key={i}
