...
export const PastelMemo = () => {
  const { filteredData } = useDashboard();

  const datosPieMemorandumsPorArea = useMemo(() => {
    if (filteredData.length === 0) return [];

    const memorandumsPorArea: Record<string, number> = {};

    // Sumar memorandums por área
    filteredData.forEach((r) => {
      memorandumsPorArea[r.area] = (memorandumsPorArea[r.area] || 0) + r.memorandums;
    });

    const total = Object.values(memorandumsPorArea).reduce((s, v) => s + v, 0);
    ...
    return Object.entries(memorandumsPorArea)
      .map(([area, value], i) => ({
  ...
  const totalMemorandums = datosPieMemorandumsPorArea.reduce(
    (s, i) => s + i.value, 0
  );
  ...
  return (
    <div className="bg-white p-6 rounded-xl shadow-md">
      <h2 className="text-xl font-bold text-slate-800 mb-4 flex items-center gap-2">
        ...
        % de Memorandums por Área
      </h2>

      {datosPieMemorandumsPorArea.length === 0 ? (
        ...
        <div className="h-[400px] w-full flex flex-col lg:flex-row items-center lg:items-start gap-6">
          ...
          <div className="w-full lg:w-2/3 h-[300px] lg:h-[400px]">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
                  data={datosPieMemorandumsPorArea}
                  cx="50%"
                  cy="50%"
                  dataKey="value"
                  innerRadius="52%"
                ...
                >
                  {datosPieMemorandumsPorArea.map((entry, index) => (
                    <Cell
                      key={`cell-${index}`}
                      fill={entry.color}
                      stroke="#fff"
                ...
                <g>
                  ...
                  <text
                    ...
                    dy={10}
                  >
                    {totalMemorandums.toLocaleString()}
                  </text>
                  <text
                    x="50%"
                    y="50%"
                ...
                <PieTooltip
                  formatter={(value) => {
                    ...
                    const perc =
                      totalMemorandums > 0
                        ? ((v / totalMemorandums) * 100).toFixed(1)
                        : "0.0";
                    return `${v} memorándums (${perc}%)`;
                  }}
                  contentStyle={{
          ...
          <div className="w-full lg:w-1/3 h-auto lg:h-[400px] overflow-y-auto pr-1">
            <div className="space-y-2">
              {datosPieMemorandumsPorArea.map((item, i) => (
                <div
                  key={i}
