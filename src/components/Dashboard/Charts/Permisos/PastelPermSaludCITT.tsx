...
export const PastelPermSaludCITT = () => {
  const { filteredData } = useDashboard();

  const datosPiePermisoSaludCITT = useMemo(() => {
    if (filteredData.length === 0) return [];

    const saludCITTPorArea: Record<string, number> = {};

    // Sumar permisoSaludCITT por área
    filteredData.forEach((r) => {
      saludCITTPorArea[r.area] = (saludCITTPorArea[r.area] || 0) + r.permisoSaludCITT;
    });

    const total = Object.values(saludCITTPorArea).reduce((s, v) => s + v, 0);
    ...
    return Object.entries(saludCITTPorArea)
      .map(([area, value], i) => ({
  ...
  const totalPermisoSaludCITT = datosPiePermisoSaludCITT.reduce(
    (s, i) => s + i.value, 0
  );
  ...
  return (
    <div className="bg-white p-6 rounded-xl shadow-md">
      <h2 className="text-xl font-bold text-slate-800 mb-4 flex items-center gap-2">
        ...
        % de Permisos Salud CITT por Área
      </h2>

      {datosPiePermisoSaludCITT.length === 0 ? (
        ...
        <div className="h-[400px] w-full flex flex-col lg:flex-row items-center lg:items-start gap-6">
          ...
          <div className="w-full lg:w-2/3 h-[300px] lg:h-[400px]">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                ...
                >
                  {datosPiePermisoSaludCITT.map((entry, index) => (
                    <Cell
                      key={`cell-${index}`}
                      fill={entry.color}
                      stroke="#fff"
                ...
                <g>
                  ...
                  <text
                    ...
                    dy={10}
                  >
                    {totalPermisoSaludCITT.toLocaleString()}
                  </text>
                  <text
                    x="50%"
                    y="50%"
                ...
                <PieTooltip
                  formatter={(value) => {
                    ...
                    const perc =
                      totalPermisoSaludCITT > 0
                        ? ((v / totalPermisoSaludCITT) * 100).toFixed(1)
                        : "0.0";
          ...
          <div className="w-full lg:w-1/3 h-auto lg:h-[400px] overflow-y-auto pr-1">
            <div className="space-y-2">
              {datosPiePermisoSaludCITT.map((item, i) => (
                <div
                  key={i}
