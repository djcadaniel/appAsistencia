...
export const PastelPermSalud2Horas = () => {
  const { filteredData } = useDashboard();

  const datosPiePermisoSalud2Horas = useMemo(() => {
    if (filteredData.length === 0) return [];

    const salud2HorasPorArea: Record<string, number> = {};

    // Sumar permisoSalud2Horas por área
    filteredData.forEach((r) => {
      salud2HorasPorArea[r.area] = (salud2HorasPorArea[r.area] || 0) + r.permisoSalud2Horas;
    });

    const total = Object.values(salud2HorasPorArea).reduce((s, v) => s + v, 0);
    ...
    return Object.entries(salud2HorasPorArea)
      .map(([area, value], i) => ({
  ...
  const totalPermisoSalud2Horas = datosPiePermisoSalud2Horas.reduce(
    (s, i) => s + i.value, 0
  );
  ...
  return (
    <div className="bg-white p-6 rounded-xl shadow-md">
      <h2 className="text-xl font-bold text-slate-800 mb-4 flex items-center gap-2">
        ...
        % de Permiso Salud 2 Horas por Área
      </h2>

      {datosPiePermisoSalud2Horas.length === 0 ? (
        ...
        <div className="h-[400px] w-full flex flex-col lg:flex-row items-center lg:items-start gap-6">
          ...
          <div className="w-full lg:w-2/3 h-[300px] lg:h-[400px]">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
                  data={datosPiePermisoSalud2Horas}
                  cx="50%"
                  cy="50%"
                  dataKey="value"
                  innerRadius="52%"
                ...
                >
                  {datosPiePermisoSalud2Horas.map((entry, index) => (
                    <Cell
                      key={`cell-${index}`}
                      fill={entry.color}
                      stroke="#fff"
                ...
                <g>
                  ...
                  <text
                    ...
                    dy={10}
                  >
                    {totalPermisoSalud2Horas.toLocaleString()}
                  </text>
                  <text
                    x="50%"
                    y="50%"
                ...
                <PieTooltip
                  formatter={(value) => {
                    ...
                    const perc =
                      totalPermisoSalud2Horas > 0
                        ? ((v / totalPermisoSalud2Horas) * 100).toFixed(1)
                        : "0.0";
          ...
          <div className="w-full lg:w-1/3 h-auto lg:h-[400px] overflow-y-auto pr-1">
            <div className="space-y-2">
              {datosPiePermisoSalud2Horas.map((item, i) => (
                <div
                  key={i}
