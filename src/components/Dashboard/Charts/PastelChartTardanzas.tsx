...
export const PastelChartTardanzas = () => {

  const { filteredData} = useDashboard();

  const datosPieTardanzasPorArea = useMemo(() => {
    if (filteredData.length === 0) return [];

    const tardanzasPorArea: Record<string, number> = {};

    filteredData.forEach((r) => {
      tardanzasPorArea[r.area] = (tardanzasPorArea[r.area] || 0) + r.totalTarde;
    });

    const total = Object.values(tardanzasPorArea).reduce((s, v) => s + v, 0);

    // Paleta simple y consistente
    const colors = ["#3b82f6", "#10b981", "#f59e0b", "#ef4444", "#8b5cf6", "#06b6d4", "#f97316", "#22c55e"];

    return Object.entries(tardanzasPorArea)
      .map(([area, value], i) => ({
        name: area,
        value,
    ...
    <>
      <div className="bg-white p-6 rounded-xl shadow-md">
        <h2 className="text-xl font-bold text-slate-800 mb-4 flex items-center gap-2">
          ...
          % de Tardanzas por Área
        </h2>

        {datosPieTardanzasPorArea.length === 0 ? (
          ...
          <div className="h-[400px] w-full flex flex-col lg:flex-row items-center lg:items-start gap-6">
            ...
            <div className="w-full lg:w-2/3 h-[300px] lg:h-[400px]">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  >
                    {datosPieTardanzasPorArea.map((entry, index) => (
                      <Cell
                        key={`cell-${index}`}
                        fill={entry.color}
                        stroke="#fff"
                  ...
                  <g>
                    ...
                    <text
                      ...
                      dy={10}
                    >
                      {datosPieTardanzasPorArea.reduce((s, i) => s + i.value, 0).toLocaleString()}
                    </text>
                    <text
                      x="50%"
                      y="50%"
                  ...
                  <PieTooltip
                    formatter={(value) => {
                      ...
                      const total = datosPieTardanzasPorArea.reduce((s, i) => s + i.value, 0);
                      const perc = total > 0 ? ((v / total) * 100).toFixed(1) : "0.0";
                      return `${v} tardanzas (${perc}%)`;
                    }}
                    contentStyle={{
            ...
            <div className="w-full lg:w-1/3 h-auto lg:h-[400px] overflow-y-auto pr-1">
              <div className="space-y-2">
                {datosPieTardanzasPorArea.map((item, i) => (
                  <div
                    key={i}
