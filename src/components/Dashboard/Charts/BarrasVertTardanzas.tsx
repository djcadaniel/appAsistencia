...
export const BarrasVertTardanzas = () => {
  ...
  const dataBarras = useMemo(() => {
    ...
    filteredData.forEach((r: any) => {
      ...
      mapa[usuario] += Number(r.totalTarde ?? 0);
    });

    return Object.entries(mapa)
      .map(([usuario, tardanzas]) => ({ usuario, tardanzas }))
      .sort((a, b) => b.tardanzas - a.tardanzas);
  }, [filteredData]);

  // ✅ altura por fila (ajusta a tu gusto)
  const ROW_HEIGHT = 32;
  const chartHeight = Math.max(420, dataBarras.length * ROW_HEIGHT);

  return (
    <div className="bg-white p-6 rounded-xl shadow-md">
      <h2 className="text-xl font-bold text-slate-800 mb-4 flex items-center gap-2">
        <TrendingUp className="w-5 h-5 text-blue-600" />
        Total de Tardanzas por Usuario
      </h2>
      ...
      {dataBarras.length === 0 ? (
        ...
        <div className="h-[420px] w-full overflow-y-auto pr-2">
          {/* ✅ El chart crece dentro del scroll */}
